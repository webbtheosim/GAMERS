...
# these variables are populated from the GAMERS inputs
...
charge_file_polymer = path + '/openff_system_generation/PEO.charges'
...
for idx_config in range(N_config) :
    ...
    system.to_lammps(dr_out+'/LAMMPS_Backup/system')
    system.to_pdb(dr_out+'/system_openff_formatting.pdb')

openmm_sys = system.to_openmm()

with open(dr_out + '/system.xml', 'w') as output:
    output.write(XmlSerializer.serialize(openmm_sys))

tpi_file_maker(dr_out + '/system.tpi',N_mol,DoP_polymer,N_atm_per_mon,smiles_head,smiles_tail)
