...
monomer.to_file(dr_out + '/{}_mon.pdb'.format(mol), file_format='pdb')
...
if 'Film' in sys_typ :
    ...
    Lx = L.to(unit.nanometer)*Lx_Lz_ratio**(1/3)
    Lz = 10*Lx/Lx_Lz_ratio
    print('box dimensions (x,y,z) <z is made 10x larger to be "nonperiodic"> [nm]:',Lx.to(unit.nanometer),Lx.to(unit.nanometer),Lz.to(unit.nanometer),flush=True)
else:
    Lx = L.to(unit.nanometer)
    Lz = L.to(unit.nanometer)
    print('box size [nm]:',L.to(unit.nanometer),flush=True)
box_dim = np.array([[Lx.m_as(unit.nanometer),0,0],[0,Lx.m_as(unit.nanometer),0],[0,0,Lz.m_as(unit.nanometer)]])*unit.nanometer
...
AllChem.EmbedMultipleConfs(rdpolymer, numConfs=1, randomSeed=(23), useRandomCoords=True)
temp = Molecule.from_rdkit(rdpolymer, allow_undefined_stereo=True)
    
for i,conformer in enumerate(temp.conformers) :
    single_config_pos = conformer

time_config = time.time()

for idx_config in range(N_config) :
    print('saving system to .pdb and LAMMPS data file for trajectory',idx_config,flush=True)
    
    atm_pos = np.zeros((1,3))
    for i in range(N_mol):
        atm_pos = np.append(atm_pos,single_config_pos + np.random.uniform(0,1,3)*Lx,axis=0)
...
with open(dr_out + '/{}.xml'.format(mol), 'w') as output:
    output.write(XmlSerializer.serialize(openmm_sys))

tpi_file_maker(dr_out + '/{}.tpi'.format(mol),N_mol,DoP_polymer,N_atm_per_mon,smiles_head,smiles_tail)
