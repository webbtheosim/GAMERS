...
- **Run Stage II:**
  ```bash
  python stage_II/phase_i.py
  python stage_II/phase_ii.py
  python stage_II/phase_iii.py
  ```
...
- `stage_II/phase_i.py` — runs the OpenMM-based Stage II simulation that consumes restraint files and produces final position/velocity files, found in `stage_II/final_positions`, for phase i of GAMERS.
- `stage_II/phase_ii.py` — runs the OpenMM-based Stage II simulation that consumes restraint files and produces final position/velocity files, found in `stage_II/final_positions`, for phase ii of GAMERS.
- `stage_II/phase_iii.py` — runs the OpenMM-based Stage II simulation that consumes restraint files and produces final position/velocity files, found in `stage_II/final_positions`, for phase iii of GAMERS.
...
## Force-field compatability with phase_x.py, x=i,ii,iii
As designed, `stage_II/phase_x.py` expect `stage_II/inputs/sys.xml` to have nonbonded forces described by a single OpenMM NonbondedForce object named "Nonbonded force".
...
If a force field requires the definition of an additional OpenMM CustomNonbondedForce object, such as CHARMM or OPLS, `stage_II/phase_x.py` must be altered.
An example for handling this senario is included in `stage_II/phase_i.py`;
commenting lines 269 and uncommenting lines 270 of `stage_II/phase_i.py` will implement the `pull_forcefield_generator_custom_force_present` python function (`stage_II/phase_i.py` line 89), which was written to properly read force-field parameters from both the NonbondedForce and CustomNonbondedForce objects.
This implementation is specifically written to handle the CustomNonbondedForce required for OPLS geometrix mixing of LJ parameters in Openmm (see `Geometric` python class in `stage_II/phase_x.py` line 14).
...
- Typical run order must be preserved: `step_1.py` → `step_2_in_maker.py` → `step_2.in` → `step_3.py` → `phase_i.py` → `phase_ii.py` → `phase_iii.py`.
