...
def kuhn_mapping(N_mon,N_mol,T,rho,M_mon,M_k,l_k):
    ...
    mon_per_tether = int(np.ceil(M_k/M_mon))
    N_tether = max(1,N_mon//mon_per_tether)
...
M_k = float(inputs['kuhn_mass(g/mol)'])*gram/mole
M_mon = float(inputs['monomer_mass(g/mol)'])*gram/mole
N_mon = int(inputs['degree_of_polymerization'])
N_mol = int(inputs['molecule_count'])
path = inputs['output_directory']

f_map = path + '/stage_I/step_1.mapping'
f_out = path + '/stage_I/step_2_inputs/step_2.data'

print('Performing kuhn mapping', flush=True)
N_b, kappa, sigma, epsilon, M_b, tau, L, T, N_tether = kuhn_mapping(N_mon,N_mol,T,rho,M_mon,M_k,l_k)
...
print('Calculating initial positions', flush=True)
positions,L = frc(kappa,N_b,N_mol)
